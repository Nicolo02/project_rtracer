...
point3_t vec3_sum_sc(point3_t one, float two) {
  point3_t two_vec = {two, two, two};
  return vec3_sum(one, two_vec);
}

point3_t vec3_mul_sc(point3_t one, float two) { // Moltip. per uno scalare
  point3_t result = {one.x * two, one.y * two, one.z * two};
  return result;
}

point3_t vec3_div_sc(point3_t one, float two) { // Divisione per uno scalare
  point3_t result = {one.x / two, one.y / two, one.z / two};
  return result;
}

point3_t vec3_sum(point3_t one, point3_t two) { // Somma tra due vettori
  point3_t result = {one.x + two.x, one.y + two.y, one.z + two.z};
  return result;
}
point3_t vec3_sub(point3_t one, point3_t two) { // Differenza tra due vettori
  point3_t result = {one.x - two.x, one.y - two.y, one.z - two.z};
  return result;
}

float vec3_len_sq(point3_t vec) {
  return vec.x * vec.x + vec.y * vec.y + vec.z * vec.z;
}

point3_t vec3_rand() {
  point3_t result = {random_float(), random_float(), random_float()};
  return result;
}

point3_t vec3_rand_range(float min, float max) {
  point3_t result = {random_float_range(min, max), random_float_range(min, max),
                   random_float_range(min, max)};
  return result;
}

float vec3_dot(point3_t u, point3_t v) {
...
point3_t vec3_rand_unit_disk() {
  while (true) {
    point3_t p = {random_float_range(-1, 1), random_float_range(-1, 1), 0};
    if (vec3_len_sq(p) >= 1) {
      continue;
    }
    return p;
...
point3_t vec3_refract(point3_t uv, point3_t n, float etai_over_etat) {
  float cos_theta = fmin(vec3_dot(vec3_mul_sc(uv, -1), n), 1.0);
  point3_t r_out_perp =
      vec3_mul_sc(vec3_sum(uv, vec3_mul_sc(n, cos_theta)), etai_over_etat);
  float r_out_parallel_scaling =
